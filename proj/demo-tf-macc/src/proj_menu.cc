...
static void do_macc(void)
{
    puts("\nExercise CFU\n");

    // init
    unsigned input_offset = 7;
    (void)cfu_op0(0, input_offset, 0);
    (void)cfu_op0_sw(0, input_offset, 0);
    printf("init: input_offset %u\n", input_offset);

    // zero accumulator
    (void)cfu_op1(0, 0, 0);
    (void)cfu_op1_sw(0, 0, 0);
    ...
    for (unsigned a = 0; a < 6; a++)
    {
        ...
        {
            unsigned cfu    = cfu_op2(0, a, b);
            unsigned cfu_sw = cfu_op2_sw(0, a, b);
    ...
    {
        unsigned cfu    = cfu_op3(0, 0, 0);
        unsigned cfu_sw = cfu_op3_sw(0, 0, 0);
        printf("final:    cfu: %4u     cfu_sw: %4u\n", cfu, cfu_sw);
    }
}
...
static struct Menu MENU = {
    ...
    "project",
    {
        ...
        MENU_ITEM('s', "sequential test", do_macc),
