...
uint32_t software_cfu(int funct3, int funct7, uint32_t rs1, uint32_t rs2) {
  ...
  switch (funct7) {
    case 0:  // Zero out buffers
      ...
      for (uint32_t in_idx = 0; in_idx < MAX_PADDED_INPUT_SIZE * MAX_INPUT_CHANNELS; ++in_idx) {
    ...
    case 16:  // Write output_shift_buffer
      ...
      printf("[cfu] output_shift_buffer[%ld] = %ld\n", address,
             (int32_t)output_shift_buffer[address]);
      return 0;

    case 17:  // Start computation
      for (int i = 0; i < output_depth; ++i) {
        printf("[cfu] >> output_shift_buffer[%d] = %ld\n", i, (int32_t) output_shift_buffer[i]);
      }
      for (int out_x = 0; out_x < input_output_width; ++out_x) {
        ...
        for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
          ...
          if (addr == 1056) {
            printf("[cfu] out_channel = %d, acc = %ld\n", out_channel, acc);
            printf("[cfu] %ld - %ld - %ld\n", acc, (int32_t)output_multiplier_buffer[out_channel],
                   (int32_t)output_shift_buffer[out_channel]);
            // printf("output_buffer[%d] = %d\n", addr, static_cast<int8_t>(acc));
          }
