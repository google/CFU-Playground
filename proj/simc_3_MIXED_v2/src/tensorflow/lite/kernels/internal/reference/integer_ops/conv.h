        ...
        for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
          ...
          for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
            ...
            for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
              ...
              //   int32_t input_val = input_data[Offset(input_shape, batch, in_y,
              //                                         in_x, 0 + group * filter_input_depth)];
              //   int32_t filter_val = filter_data[Offset(
              //       filter_shape, out_channel, filter_y, filter_x, 0)];
              //   acc += filter_val * (input_val + input_offset);

              //   input_val = input_data[Offset(input_shape, batch, in_y, in_x,
              //                                 1 + group * filter_input_depth)];
              //   filter_val =
              //       filter_data[Offset(filter_shape, out_channel, filter_y,
              //                          filter_x, 1)];
              //   acc += filter_val * (input_val + input_offset);
              ...
              if (input_depth % 4 == 0) {
                  for (int in_channel = 0; in_channel < input_depth;
                    ...
                    int32_t input_val = *(uint32_t* )(input_data + Offset(input_shape, batch, in_y,
                                                          in_x, in_channel + group * filter_input_depth));
                    int32_t filter_val = *(uint32_t* )(filter_data + Offset(
                        filter_shape, out_channel, filter_y, filter_x, in_channel));
