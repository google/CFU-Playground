...
void fb_init() {
	...
	printf("\r\nFB Width x Height : %dx%d@0x%08X\r\n", FB_WIDTH, FB_HEIGHT, FB_BASE_ADDR);

    if(!dotfont_initilized) dotfont_init();
}
...
int32_t fb_fill_rect(uint32_t left, uint32_t top, uint32_t width, uint32_t height, uint32_t color) {
    ...
    for(i = 0; i < width ; i++) bar_line[i] = color;

    fb_offset = top * FB_WIDTH + left;
    fb32_ptr = (uint32_t*)FB_BASE_ADDR;
    fb32_ptr = fb32_ptr + fb_offset;
    for(i = 0; i < height; i++){
        memcpy((uint8_t*)fb32_ptr, (uint8_t*)bar_line, (width * sizeof(uint32_t)));
        fb32_ptr += FB_WIDTH;
    }
...
int32_t fb_draw_string(uint32_t x, uint32_t y, uint32_t color, const char *msg_str) {
	...
	c_pos = 0;
	font_y_min = y;
	font_y_max = ((int32_t)y + font_height) > height ?  (height - (int32_t)y) : ((int32_t)y + font_height);

	while(1) {
		if(msg_str[c_pos] == 0x00) break;
		current_char = msg_str[c_pos];
		font_data_ptr = df_info.font_data + (current_char - ' ') * (font_height * font_offset);

		font_x_min = x + c_pos * font_width;
		font_x_max = (font_x_min + font_width) > width ?  (height - font_x_min) : (font_x_min + font_width);
		...
		for(i = 0, j = font_y_min; j < font_y_max; j++, i += 2) {
			f_basis = (struct font_basis *)&df_info.f_basis[font_data_ptr[i + 0]];
			for(l = 0, k =  0; k <  8; k++, l++) {
				//printf("%d ", font_x_min + k);
				if(f_basis->data[l] == 'X') *(dst_ptr + j * width + font_x_min + k) =  color;
			}

			f_basis = (struct font_basis *)&df_info.f_basis[font_data_ptr[i + 1]];
